...
router.post("/analyze/:userId", authMiddleware, rateLimiter, quotaMiddleware, validatePrompt, async (req: any, res: any) => {
  try {
    ...
    const response = await runAI("compta", req.body);
    
    // LOG USAGE (Analyse comptable souvent gourmande en tokens)
    const inputStr = JSON.stringify(req.body);
    const outputStr = JSON.stringify(response);
    await quotaService.recordUsage(userId, "compta", inputStr, outputStr).catch(e => console.error("Usage log error:", e));
