...
export const DevisForm = () => {
  ...
  const handleAiAnalysis = (data: any) => {
    if (data?.client) setClientName(String(data.client));
  ...
  const updateItem = (index: number, field: keyof DevisItem, value: string) => {
    ...
    newItems[index] = { ...current, [field]: processed } as DevisItem;
    setItems(newItems);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetchAuth("/devis", {
        method: "POST",
      ...
      if (!response.ok) {
        // best-effort: récupérer message éventuel
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-8">
          ...
          <div className="pt-8 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <button
              ...
              className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white font-black px-8 py-4 rounded-2xl shadow-lg transition-colors flex items-center justify-center gap-3 disabled:opacity-60"
            >
              {isSubmitting ? (
                <>
